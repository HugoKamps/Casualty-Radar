...
namespace Casualty_Radar.Models.DataControl.Graph.Ways {
    ...
    public class WayTypeControl {
        ...
        public WayType ParseWayType(string key) {
            switch (key) {
                case "liv":
                    return WayType.LivingStreet;
                case "mot":
                    return WayType.MotorWay;
                case "mot_l":
                    return WayType.MotorWayLink;
                case "pth":
                    return WayType.Path;
                case "pri_l":
                    return WayType.PrimaryLink;
                case "pri":
                    return WayType.PrimaryWay;
                case "res":
                    return WayType.ResidentialWay;
                case "sec_l":
                    return WayType.SecondaryLink;
                case "sec":
                    return WayType.SecondaryWay;
                case "ter_l":
                    return WayType.TertiaryLink;
                case "ter":
                    return WayType.TertiaryWay;
                case "tru":
                    return WayType.Trunk;
                case "tru_l":
                    return WayType.TrunkLink;
                default:
                case "unc":
                    return WayType.UnclassifiedWay;
            }
        }
        ...
        public IEnumerable<Way> GetByZoomLevel(WayZoomLevel level)
            => _collection.Ways.Select(x => x).Where(x => (int) x.WayType == (int) level);
    }
}